...
function JsonTable(props) {
  ...
  [tableInfo, updateTableInfo] = (0, _react.useState)(null),
    ...
    props.columns.forEach((item, idx) => {
      ...
      p.push( // [`columns[${idx}][data]`, item.data],
      [`columns[${idx}][name]`, item.data], [`columns[${idx}][searchable]`, item.searchable !== undefined ? item.searchable.toString() : 'true'], [`columns[${idx}][orderable]`, item.orderable !== undefined ? item.searchable.toString() : 'true'] // [`columns[${idx}][search][value]`, ''],
  ...
  function updateUrl() {
    ...
    let url_params = [...column_url_params, 'draw=1', `start=${start}`, `length=${props.pageRows}`, `search[value]=${encodeURIComponent(searchString ?? '')}` // `_=${Date.now()}`
    ];
  ...
  (0, _react.useEffect)(() => {
    updateUrl(); // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []); // =>> loading json
  ...
  }, [url, props.defaultContents]); // =>> costruzione tabella e paginazione
    ...
    const TableRows = () => {
      return jsonData.data.map((row, idx) => {
        return /*#__PURE__*/_react.default.createElement("tr", {
          ...
          let cell_content; // =>> generazione tabella: tipo dati
