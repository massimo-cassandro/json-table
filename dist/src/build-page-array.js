...
function _default(currentPage, pageRows, records) {
  ...
  if (pages <= showed_pages) {
    ...
    slots_left = step_min > 2 ? 2 : step_min - 1,
        slots_right = step_max < pages - 1 ? 2 : pages - step_max; // console.log(step_min, step_max, slots_left, slots_right);
    ...
    pages_array = [...Array(step_max - step_min + 1).fill().map((_, idx) => step_min + idx)]; // console.log(pages_array.slice(0));
